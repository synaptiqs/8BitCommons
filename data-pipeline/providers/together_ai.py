"""
Together AI scraper — pricing page live check.
Live: fetches Together AI pricing page for dedicated GPU cluster rates.
Together AI is primarily an inference platform but offers dedicated GPU clusters.
"""
import re
...
HOME_URL    = "https://www.together.ai"
...
class Scraper(BaseScraper):
    ...
    def _scrape_price(self) -> Optional[float]:
        ...
        for m in _PRICE_PAT.finditer(html):
            ...
            if raw:
                price = float(raw)
                # Together AI pricing page also shows inference $/M-token prices
                # GPU compute should be between $1–$10/GPU/hr; token prices won't be
                if 1.0 < price < 10.0:
                    return price
        return None
